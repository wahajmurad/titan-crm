      ...
      {loading ? (
        <div className="grid gap-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-20 bg-gray-100 rounded-2xl animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
          ))}
        </div>
      ) : meetings.length === 0 ? (
        <motion.div
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div className={cn(
              ...
              meeting.status === 'SCHEDULED'
                ? 'bg-gradient-to-br from-blue-500 to-blue-600 text-white border-blue-500 shadow-sm shadow-blue-500/20'
                : config.bg
            )}>
