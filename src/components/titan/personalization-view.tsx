...
export function PersonalizationView() {
  ...
  return (
    <motion.div className="mx-auto max-w-6xl space-y-6" variants={container} initial="hidden" animate="show">
      {/* ═══ HEADER ═══ */}
      <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-gradient-to-br from-rose-500 to-pink-600 rounded-xl flex items-center justify-center shadow-sm shadow-rose-500/20">
              <Heart className="w-4 h-4 text-white" fill="white" />
            </div>
            <h1 className="text-2xl font-bold tracking-tight">Hyper-Personalization Engine</h1>
          </div>
        ...
        <Badge variant="outline" className="bg-rose-50 text-rose-700 border-rose-200 text-xs font-semibold px-3 py-1 self-start sm:self-auto">
      ...
      <motion.div variants={stagger}>
        <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button
              ...
              className="text-xs text-gray-500 hover:text-rose-600 h-7 px-2"
            >
            ...
            {quickMode ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      className="h-10 rounded-xl bg-gray-50 border-0 text-sm"
                    />
                  </div>
                  <div>
              ...
              <motion.div key="lead" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}>
                <Select value={selectedLead} onValueChange={setSelectedLead}>
                  <SelectTrigger className="h-10 rounded-xl bg-gray-50 border-0 text-sm">
          ...
          <div className="mt-4 flex items-center gap-3">
            <Button
              ...
              className="h-10 bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white rounded-xl font-semibold shadow-sm shadow-rose-500/20 px-6"
            >
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="outline" size="sm" onClick={generateLinkedIn} disabled={!!generatingAsset} className="h-9 rounded-lg text-xs">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="outline" size="sm" onClick={generateBlueprint} disabled={!!generatingAsset} className="h-9 rounded-lg text-xs">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="outline" size="sm" onClick={generateROI} disabled={!!generatingAsset} className="h-9 rounded-lg text-xs">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="outline" size="sm" onClick={generatePackage} disabled={!!generatingAsset} className="h-9 rounded-lg text-xs">
      ...
      <AnimatePresence>
        {(selectedLeadData || (quickMode && qsCompany)) && (
          <motion.div variants={stagger} initial="hidden" animate="show" exit="hidden">
            <Card className="bg-gradient-to-r from-rose-50/80 to-pink-50/80 dark:from-rose-950/20 dark:to-pink-950/20 rounded-2xl border-0 p-4">
              <div className="flex items-center gap-4 flex-wrap">
                <div>
                  <div className="text-[11px] text-gray-500 font-medium uppercase tracking-wide">Company</div>
                  <div className="font-bold text-gray-900 dark:text-gray-100">{quickMode ? qsCompany : selectedLeadData?.businessName}</div>
                </div>
                {(quickMode ? qsIndustry : selectedLeadData?.industry) && (
                  <div>
                    ...
                    <div className="text-sm text-gray-700 dark:text-gray-300">{quickMode ? qsIndustry : selectedLeadData?.industry}</div>
                  </div>
                )}
      ...
      {(selectedLead || (quickMode && qsCompany)) && pipelineSteps.length > 0 && (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-gray-100 dark:bg-gray-800 rounded-xl p-1 h-auto flex-wrap gap-0.5">
          ...
          <TabsContent value="pipeline" className="mt-6 space-y-4">
            {pipelineRunning && (
              <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-4">
            ...
            {pipelineSteps.map((step, idx) => {
              ...
              return (
                ...
                >
                  <Card
                    className={cn(
                      'rounded-2xl border-0 shadow-sm transition-all cursor-pointer hover:shadow-md',
                      step.status === 'completed' && 'bg-white dark:bg-gray-900',
                      step.status === 'running' && 'bg-blue-50/50 ring-1 ring-blue-200 dark:bg-blue-950/20',
                      step.status === 'failed' && 'bg-red-50/50 dark:bg-red-950/20',
                      step.status === 'skipped' && 'bg-gray-50 opacity-60 dark:bg-gray-800/50',
                    )}
                      ...
                      {isExpanded && step.output && (
                        ...
                        >
                          <div className="px-4 pb-4 border-t border-gray-100 dark:border-gray-800 pt-3">
                            <pre className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-xl p-3 overflow-auto max-h-48 whitespace-pre-wrap">
            ...
            {email ? (
              <>
                ...
                <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-5">
                  ...
                  {isEditing ? (
                    <Input value={editedSubject} onChange={e => setEditedSubject(e.target.value)} className="rounded-lg bg-gray-50 border-0 text-sm font-semibold" />
                  ) : (
                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{email.subject}</div>
                  )}
                </Card>
                ...
                <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-5">
                  ...
                  {isEditing ? (
                    <Textarea value={editedBody} onChange={e => setEditedBody(e.target.value)} className="rounded-xl bg-gray-50 border-0 text-sm min-h-[200px] resize-none" />
                  ) : (
                    <div ref={emailRef} className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap bg-gray-50/50 dark:bg-gray-800/50 rounded-xl p-4">
                      {email.body}
                    </div>
                  )}
                </Card>
                ...
                {selectedLead && (
                  <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-5">
                    ...
                    <Textarea
                      ...
                      className="rounded-xl bg-gray-50 border-0 text-sm min-h-[60px] resize-none mb-3"
                    />
                    <div className="flex items-center gap-2">
                      <Button
                        onClick={handleApprove}
                        disabled={!!generatingAsset}
                        className="h-9 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold px-4"
                      >
                      ...
                      <Button
                        ...
                        disabled={!!generatingAsset}
                        variant="outline"
                        className="h-9 rounded-xl text-xs px-4 border-red-200 text-red-600 hover:bg-red-50"
                      >
              ...
              <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-12 text-center">
          ...
          <TabsContent value="linkedin" className="mt-6 space-y-4">
            {linkedin ? (
                      ...
                      {(linkedin.followUpSequence as Array<Record<string, unknown>>).map((step, i) => (
                        <div key={i} className="flex gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-xs font-semibold text-gray-900 dark:text-gray-100">{String(step.action || '').replace(/_/g, ' ')}</span>
                              <Badge variant="outline" className="text-[10px] h-4">{String(step.day || '')}</Badge>
                            </div>
                            <p className="text-xs text-gray-600 dark:text-gray-400">{String(step.content || '')}</p>
              ...
              <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-12 text-center">
                ...
                {pipelineDone && (
                  <Button variant="outline" onClick={generateLinkedIn} disabled={!!generatingAsset} className="rounded-xl text-xs">
            ...
            {salesAngles.length > 0 ? (
              <div className="grid gap-3">
                {salesAngles.map((angle, idx) => (
                  ...
                  >
                    <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-5 hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1.5">
                            <span className="text-xs font-bold text-gray-400">#{idx + 1}</span>
                            <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{angle.angleName}</span>
                            <Badge variant="outline" className={cn(
                              'text-[10px] h-5',
                          ...
                          <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">{angle.hook}</p>
                          <div className="space-y-1.5">
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <span className="text-xs text-gray-700 dark:text-gray-300">{angle.subjectLine}</span>
                              </div>
                            </div>
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <span className="text-xs text-gray-700 dark:text-gray-300">{angle.openingLine}</span>
                              </div>
                            </div>
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <span className="text-xs text-gray-700 dark:text-gray-300">{angle.valueProposition}</span>
                              </div>
                            </div>
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <span className="text-xs text-gray-700 dark:text-gray-300">{angle.emotionalTrigger}</span>
                              </div>
                            </div>
                          </div>
                        </div>
          ...
          <TabsContent value="blueprint" className="mt-6 space-y-4">
            {blueprint ? (
              <>
                {blueprint.currentSituation && (
                  <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-5">
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{String(blueprint.currentSituation)}</p>
                  </Card>
                )}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {(['thirtyDayPlan', 'sixtyDayPlan', 'ninetyDayPlan'] as const).map((plan, i) => {
                    ...
                    if (!data) return null
                    const colors = ['border-blue-400 text-blue-700', 'border-purple-400 text-purple-700', 'border-emerald-400 text-emerald-700']
                    const labels = ['Days 1-30', 'Days 31-60', 'Days 61-90']
                    return (
                      <Card key={plan} className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-5 border-l-4" style={{ borderLeftColor: colors[i].includes('blue') ? '#3B82F6' : colors[i].includes('purple') ? '#8B5CF6' : '#059669' }}>
                        <Badge variant="outline" className={cn('text-[10px] mb-2', colors[i])}>{labels[i]}</Badge>
                        <div className="text-sm font-bold text-gray-900 dark:text-gray-100 mb-2">{String(data.focus || '')}</div>
                        <p className="text-xs text-gray-600 dark:text-gray-400">{String(data.expectedResults || '')}</p>
                      </Card>
                    )
                  })}
                </div>
                    ...
                    <div className="space-y-2">
                      {(blueprint.recommendedAISolutions as Array<Record<string, unknown>>).slice(0, 5).map((s, i) => (
                        <div key={i} className="flex items-start gap-2 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
                          ...
                          <div>
                            <div className="text-xs font-semibold text-gray-900 dark:text-gray-100">{String(s.solution || s.name || '')}</div>
              ...
              <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-12 text-center">
                ...
                {pipelineDone && (
                  <Button variant="outline" onClick={generateBlueprint} disabled={!!generatingAsset} className="rounded-xl text-xs">
          ...
          <TabsContent value="roi" className="mt-6 space-y-4">
            {roiData ? (
              <>
                {roiData.summary && (
                  <Card className="bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-950/20 dark:to-green-950/20 rounded-2xl border-0 p-5">
                    <p className="text-sm text-emerald-800 dark:text-emerald-200 font-medium">{String(roiData.summary)}</p>
                    {roiData.confidence && (
                      <Badge variant="outline" className="mt-2 text-[10px] bg-white/80 border-emerald-200 text-emerald-700">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    ...
                    { label: 'Cost Reduction', value: roiData.costReduction?.percent ? `${roiData.costReduction.percent}%` : roiData.potentialCostReduction || 'N/A', color: 'text-purple-600', icon: DollarSign },
                    { label: 'Payback Period', value: roiData.paybackPeriod || 'N/A', color: 'text-amber-600', icon: Target },
                  ].map(m => (
                    <Card key={m.label} className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-4 text-center">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {roiData.implementationCost && (
                        <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                          ...
                          <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{String(roiData.implementationCost.oneTime || 'N/A')}</div>
                          <div className="text-[10px] text-gray-400 font-medium mt-2">Monthly Cost</div>
                          <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{String(roiData.implementationCost.monthly || 'N/A')}</div>
                        </div>
                      )}
                      {roiData.roi && (
                        <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                      ...
                      {roiData.netValue && (
                        <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                          ...
                          <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{String(roiData.netValue.year1 || 'N/A')}</div>
                          <div className="text-[10px] text-gray-400 font-medium mt-2">Year 2 Net Value</div>
                          <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{String(roiData.netValue.year2 || 'N/A')}</div>
                        </div>
                      )}
                    </div>
                  </Card>
              ...
              <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-12 text-center">
                ...
                {pipelineDone && (
                  <Button variant="outline" onClick={generateROI} disabled={!!generatingAsset} className="rounded-xl text-xs">
            ...
            {selectedLead ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <Card key={asset.type} className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-5 hover:shadow-md transition-shadow cursor-pointer group" onClick={() => generateAsset(asset.type)}>
                    <div className="flex items-center gap-3">
                      <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center', asset.color)}>
                        <asset.icon className="w-5 h-5" />
                      </div>
                      <div className="flex-1">
                        <div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{asset.label}</div>
          ...
          <TabsContent value="package" className="mt-6 space-y-4">
            {outreachPackage ? (
              <>
                {/* Tier badge */}
                <Card className={cn('rounded-2xl border-0 p-5', TIER_BG[outreachPackage.tier])}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-lg font-bold text-gray-900 dark:text-gray-100">{outreachPackage.estimatedDealValue || 'N/A'}</div>
                    </div>
                  </div>
                  <p className="text-sm text-gray-700 dark:text-gray-300 mt-3">{outreachPackage.rationale}</p>
                </Card>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  {outreachPackage.totalAssetsToGenerate && (
                    <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-4 text-center">
                      <div className="text-xl font-black text-purple-600">{outreachPackage.totalAssetsToGenerate}</div>
                      ...
                      {outreachPackage.recommendedSequence.map((seq, i) => (
                        <div key={i} className="flex gap-3">
                          <div className="flex flex-col items-center">
                            <div className="w-7 h-7 bg-gradient-to-br from-rose-500 to-pink-500 rounded-full flex items-center justify-center text-white text-[10px] font-bold">{seq.step}</div>
                          ...
                          <div className="flex-1 pb-3">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-xs font-semibold text-gray-900 dark:text-gray-100">{seq.action}</span>
                              <Badge variant="outline" className="text-[10px] h-4">{seq.timing}</Badge>
                            </div>
                            <p className="text-xs text-gray-600 dark:text-gray-400">{seq.reason}</p>
                            {seq.asset && (
              ...
              <Card className="bg-white dark:bg-gray-900 rounded-2xl border-0 shadow-sm p-12 text-center">
                ...
                {pipelineDone && (
                  <Button variant="outline" onClick={generatePackage} disabled={!!generatingAsset} className="rounded-xl text-xs">
