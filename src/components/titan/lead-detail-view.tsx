...
function ScoreGauge({ score }: { score: number }) {
 ...
 return (
  <Card className="bg-white rounded-xl border-0 shadow-sm p-6">
   <p className="text-xs font-semibold text-[gray-400] uppercase tracking-wider mb-4">Score</p>
   <div className="flex flex-col items-center">
    <div className="relative">
     <svg width={size} height={size} className="-rotate-90">
      <circle
       cx={size / 2} cy={size / 2} r={radius}
       fill="none" stroke="gray-50" strokeWidth={strokeWidth}
      />
      <motion.circle
       cx={size / 2} cy={size / 2} r={radius}
     ...
     <div className="absolute inset-0 flex flex-col items-center justify-center">
      <span className="text-2xl font-bold text-[gray-900] tabular-nums">{clampedScore}</span>
...
function MeetingStatusBadge({ status }: { status: string }) {
 ...
 return (
  <Badge variant="outline" className={cn('text-[11px] font-semibold rounded-lg px-2 py-0.5', styles[status] || 'bg-gray-50 text-gray-600 border-gray-200')}>
   {status}
  </Badge>
 )
}
...
export function LeadDetailView() {
 ...
 if (!lead) return <p className="text-[gray-500] py-12 text-center font-medium">Lead not found</p>
 ...
 className="text-[gray-400] hover:text-[gray-900] hover:bg-gray-100 rounded-xl h-9 w-9 p-0 flex items-center justify-center shrink-0"
     >
      <ArrowLeft className="w-4 h-4" />
     </Button>
     <div className="min-w-0">
      <div className="flex items-center gap-3 flex-wrap">
       <h1 className="text-2xl font-bold text-[gray-900] tracking-tight truncate">{b.name}</h1>
       {b.industry && (
 ...
 className="text-red-500 hover:text-red-600 hover:bg-red-50 border-red-200 rounded-xl h-9 px-3 font-semibold shrink-0"
   ...
   <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
    <div className="lg:col-span-2 space-y-5">
     ...
     <motion.div variants={fadeUp}>
      <Card className="bg-white rounded-xl border-0 shadow-sm p-6">
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {b.website && (
         <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 sm:col-span-2 group">
          <div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center shrink-0">
           <Globe className="w-4 h-4 text-blue-600" />
          </div>
          <a href={b.website} target="_blank" rel="noopener" className="text-sm text-blue-600 hover:underline truncate font-medium">
           {b.website}
          </a>
          <ExternalLink className="w-3.5 h-3.5 text-[gray-400] shrink-0 group-hover:text-blue-600 transition-colors" />
         </div>
        )}
        {b.phone && (
         <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
          ...
          <span className="text-sm text-[gray-900] font-medium">{b.phone}</span>
         </div>
        )}
        {b.email && (
         <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
          ...
          <span className="text-sm text-[gray-900] font-medium">{b.email}</span>
         </div>
        )}
        {b.companySize && (
         <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
          ...
          <span className="text-sm text-[gray-900] font-medium">{b.companySize}</span>
         </div>
        )}
        {b.source && (
         <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
          ...
          <span className="text-sm text-[gray-500]">Source: <span className="text-[gray-900] font-medium">{b.source}</span></span>
         </div>
        )}
        {b.businessCategory && (
         <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
          ...
          <span className="text-sm text-[gray-900] font-medium">{b.businessCategory}</span>
         </div>
        )}
       </div>
      </Card>
     ...
     <motion.div variants={fadeUp}>
      <Card className="bg-white rounded-xl border-0 shadow-sm p-6">
       <h3 className="text-sm font-semibold text-[gray-900] mb-4">Decision Maker</h3>
       {lead.decisionMaker ? (
        <div className="flex items-center gap-4">
         <div className="relative">
          <div className="w-14 h-14 rounded-xl bg-gradient-to-br to-blue-600 flex items-center justify-center text-white text-lg font-bold shadow-md ring-4 ring-blue-50">
           {lead.decisionMaker.slice(0, 2).toUpperCase()}
          </div>
         </div>
         <div>
          <p className="text-base font-bold text-[gray-900]">{lead.decisionMaker}</p>
          {lead.decisionMakerRole && (
           <p className="text-sm text-[gray-400] mt-0.5">{lead.decisionMakerRole}</p>
          )}
          {lead.decisionMakerEmail && (
           <p className="text-sm text-blue-600 mt-1.5 flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-3 py-2">
         ...
         <div>
          <p className="text-sm text-[gray-400] font-medium">Not identified yet</p>
          <p className="text-xs text-[gray-400] mt-0.5">Use AI qualification to find decision makers</p>
         </div>
        </div>
       )}
      </Card>
     ...
     {lead.recommendedSolution && (
      <motion.div variants={fadeUp}>
       <Card className="bg-white rounded-xl border-0 shadow-sm p-6">
        <h3 className="text-sm font-semibold text-[gray-900] mb-3">Recommended Solution</h3>
        <p className="text-sm text-[gray-500] whitespace-pre-wrap leading-relaxed">{lead.recommendedSolution}</p>
       </Card>
      </motion.div>
     )}
      ...
      <Card className="bg-white rounded-xl border-0 shadow-sm p-6">
       <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
         ...
         <h3 className="text-sm font-semibold text-[gray-900]">Outreach History</h3>
        </div>
        <span className="text-xs text-[gray-400] font-medium bg-gray-50 rounded-full px-2.5 py-0.5 tabular-nums">{lead.outreaches.length}</span>
       </div>
          ...
          {lead.outreaches.map(o => {
           ...
           return (
            <motion.div
             key={o.id}
             variants={stagger}
 className={cn('border-l-[3px] pl-4 py-3 hover:bg-gray-50/80 rounded-r-xl transition-colors', statusColors[o.status] || 'border-l-slate-300')}
            >
             <div className="flex items-center gap-2 mb-1">
              <span className="text-sm font-semibold text-[gray-900]">{o.subject}</span>
              <OutreachStatusBadge status={o.status} />
             </div>
             <p className="text-xs text-[gray-400] line-clamp-2 leading-relaxed">{o.body}</p>
     ...
     <motion.div variants={fadeUp}>
      <Card className="bg-white rounded-xl border-0 shadow-sm p-6">
       <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-[gray-400] font-medium bg-gray-50 rounded-full px-2.5 py-0.5 tabular-nums">{lead.meetings.length}</span>
       </div>
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-3">
         {lead.meetings.map(m => (
          <motion.div
           key={m.id}
           variants={stagger}
 className="flex items-center justify-between gap-3 bg-gray-50/80 rounded-xl px-4 py-3.5 hover:bg-gray-100/80 transition-colors flex-wrap sm:flex-nowrap"
          >
    ...
    <div className="space-y-5">
     ...
     <motion.div variants={fadeUp}>
      <Card className="bg-white rounded-xl border-0 shadow-sm p-6">
       <p className="text-xs font-semibold text-[gray-400] uppercase tracking-wider mb-3">Current Stage</p>
       {editingStage ? (
        <Select defaultValue={lead.stage} onValueChange={v => updateStage(v)}>
         <SelectTrigger className="rounded-xl border-0 bg-gray-50 h-10">
          <SelectValue />
         </SelectTrigger>
         <SelectContent>
 ...
 className={cn('text-sm font-semibold rounded-xl px-3.5 py-1.5 transition-all group-hover:shadow-sm', STAGE_COLORS[lead.stage])}
         ...
         <p className="text-[11px] text-[gray-400] mt-2">Click to change stage</p>
        </div>
       )}
      </Card>
     </motion.div>
     ...
     <motion.div variants={fadeUp}>
      <Card
 className="bg-gradient-to-br from-rose-50 to-pink-50 rounded-xl border-0 shadow-sm p-5 cursor-pointer hover:shadow-md transition-shadow group"
        ...
        {lead.activities.length === 0 ? (
         ...
         lead.activities.map(a => (
          <motion.div key={a.id} variants={stagger} className="text-xs">
           <p className="text-[gray-500] font-medium leading-relaxed">{a.details}</p>
     ...
     <motion.div variants={fadeUp}>
      <Card className="bg-white rounded-xl border-0 shadow-sm p-6">
       ...
       {lead.notes && (
        <div className="bg-gray-50 rounded-xl p-3.5 mb-4 max-h-40 overflow-y-auto">
         <p className="text-xs text-[gray-500] whitespace-pre-wrap leading-relaxed">{lead.notes}</p>
        </div>
       )}
       <Textarea
 ...
 className="text-sm min-h-[80px] rounded-xl bg-gray-50 border-0 focus-visible:ring-1 focus-visible:ring-[#2563EB]/30 resize-none placeholder:text-[gray-400]"
       />
       <Button
        size="sm"
 className="w-full mt-3 h-10 rounded-xl bg-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold shadow-sm hover:shadow-md transition-all disabled:opacity-50"
        onClick={updateNotes}
