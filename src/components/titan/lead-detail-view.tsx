...
export function LeadDetailView() {
  ...
  if (!lead) return <p className="text-slate-400 py-8 text-center">Lead not found</p>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => setView('leads')} className="text-slate-500">
        ...
        <div className="flex-1">
          ...
          {b.city && <p className="text-sm text-slate-500 mt-0.5 flex items-center gap-1"><MapPin className="w-3 h-3" />{b.city}{b.country ? `, ${b.country}` : ''}</p>}
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card className="border-slate-200">
            ...
            <CardContent className="grid grid-cols-2 gap-3 text-sm">
              {b.website && <div className="flex items-center gap-2 text-slate-600 col-span-2"><Globe className="w-3.5 h-3.5 text-slate-400" /><a href={b.website} target="_blank" rel="noopener" className="text-blue-600 hover:underline truncate">{b.website}</a></div>}
              {b.phone && <div className="flex items-center gap-2 text-slate-600"><Phone className="w-3.5 h-3.5 text-slate-400" />{b.phone}</div>}
              {b.email && <div className="flex items-center gap-2 text-slate-600"><Mail className="w-3.5 h-3.5 text-slate-400" />{b.email}</div>}
              {b.companySize && <div className="flex items-center gap-2 text-slate-600"><Building2 className="w-3.5 h-3.5 text-slate-400" />{b.companySize}</div>}
              {b.source && <div className="flex items-center gap-2 text-slate-600"><ExternalLink className="w-3.5 h-3.5 text-slate-400" />Source: {b.source}</div>}
            </CardContent>
          </Card>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {lead.decisionMakerRole && <p className="text-xs text-slate-500">{lead.decisionMakerRole}</p>}
                    </div>
                  </div>
                  {lead.decisionMakerEmail && <p className="text-sm text-slate-600 flex items-center gap-2 mt-2"><Mail className="w-3.5 h-3.5" />{lead.decisionMakerEmail}</p>}
                </div>
          ...
          <Card className="border-slate-200">
            ...
            <CardContent>
              {lead.outreaches.length === 0 ? <p className="text-sm text-slate-400">No outreach yet</p> : (
                <div className="space-y-3 max-h-64 overflow-y-auto">
                  {lead.outreaches.map(o => (
                    <div key={o.id} className="border-l-2 border-slate-200 pl-3 py-1">
                      ...
                      <p className="text-xs text-slate-500 mt-1 line-clamp-2">{o.body}</p>
                      <p className="text-xs text-slate-400 mt-1">{o.sentAt ? format(new Date(o.sentAt), 'MMM d, h:mm a') : 'Draft'} · {o.type.replace(/_/g, ' ')}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          <Card className="border-slate-200">
            ...
            <CardContent>
              {lead.meetings.length === 0 ? <p className="text-sm text-slate-400">No meetings yet</p> : (
                <div className="space-y-2">
                  {lead.meetings.map(m => (
                    <div key={m.id} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                      <div>
                        <p className="text-sm font-medium text-slate-900">{m.title}</p>
                        <p className="text-xs text-slate-500">{format(new Date(m.date), 'MMM d, yyyy')} · {m.duration}min</p>
                      </div>
        ...
        <div className="space-y-4">
          <Card className="border-slate-200">
            <CardContent className="p-4">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">Current Stage</p>
              {editingStage ? (
          ...
          <Card className="border-slate-200">
            <CardContent className="p-4">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">Score</p>
              <p className="text-2xl font-bold text-slate-900">{lead.score}<span className="text-sm text-slate-400 font-normal">/100</span></p>
            </CardContent>
          </Card>
              ...
              <div className="space-y-2.5 max-h-48 overflow-y-auto">
                {lead.activities.map(a => (
                  <div key={a.id} className="text-xs">
                    ...
                    <p className="text-slate-400 mt-0.5">{a.user?.name} · {format(new Date(a.createdAt), 'MMM d, h:mm a')}</p>
                  </div>
                ))}
                {lead.activities.length === 0 && <p className="text-xs text-slate-400">No activity</p>}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200">
            ...
            <CardContent>
              {lead.notes && <p className="text-xs text-slate-600 whitespace-pre-wrap mb-3 max-h-32 overflow-y-auto">{lead.notes}</p>}
