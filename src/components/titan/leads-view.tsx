    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{leads.length} leads in pipeline</p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider">Industry</th>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider">Emails</th>
                ...
                <tbody>
                  {leads.length === 0 ? (
                    <tr><td colSpan={7} className="px-4 py-12 text-center text-slate-400">No leads yet. Add your first lead to get started.</td></tr>
                  ) : leads.map(lead => (
                    <tr key={lead.id} className="border-b border-slate-50 hover:bg-slate-50/50 cursor-pointer transition-colors" onClick={() => setView('lead-detail', lead.id)}>
                      <td className="px-4 py-3">
                        ...
                        {lead.business.city && <div className="text-xs text-slate-400 mt-0.5">{lead.business.city}{lead.business.country ? `, ${lead.business.country}` : ''}</div>}
                      </td>
                      <td className="px-4 py-3 text-slate-600">{lead.business.industry || '—'}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        {lead.decisionMaker ? (
                          <div>
                            ...
                            {lead.decisionMakerRole && <div className="text-xs text-slate-400">{lead.decisionMakerRole}</div>}
                          </div>
                        ) : <span className="text-slate-300">—</span>}
                      </td>
                      <td className="px-4 py-3 text-slate-600">{lead._count.outreaches}</td>
                      <td className="px-4 py-3 text-slate-400 text-xs">{format(new Date(lead.updatedAt), 'MMM d')}</td>
            ...
            return (
              <div key={stage} className="min-w-64 flex-1">
                <div className="flex items-center gap-2 mb-3 px-1">
                  ...
                  <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">{stage.replace(/_/g, ' ')}</span>
                  <span className="text-xs text-slate-300">{stageLeads.length}</span>
                </div>
                  ...
                  {stageLeads.map(lead => (
                    ...
                    >
                      <CardContent className="p-3">
                        ...
                        {lead.business.industry && <p className="text-xs text-slate-400 mt-1">{lead.business.industry}</p>}
                        {lead.decisionMaker && <p className="text-xs text-slate-500 mt-1.5">{lead.decisionMaker}</p>}
                        <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-50">
                          <Mail className="w-3 h-3 text-slate-300" />
                          <span className="text-xs text-slate-400">{lead._count.outreaches}</span>
                          <Calendar className="w-3 h-3 text-slate-300 ml-2" />
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="grid grid-cols-2 gap-3 mt-2">
          ...
          <div className="col-span-2 border-t border-slate-100 pt-3 mt-1"><p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Decision Maker</p></div>
