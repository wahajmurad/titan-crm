...
export function OutreachView() {
   ...
   {loading ? (
    <div className="grid gap-3">
     {Array.from({ length: 4 }).map((_, i) => (
      <div key={i} className="h-16 bg-gray-100 rounded-xl animate-pulse" style={{ animationDelay: `${i * 80}ms` }} />
     ))}
    </div>
   ) : filtered.length === 0 ? (
    <motion.div
 ...
 className="rounded-xl border border-gray-100 bg-white/80 p-4 hover:border-blue-100 transition-colors group"
       >
...
function ComposeDialog({ open, onOpenChange, leads, onSent }: {
 ...
 className="rounded-xl bg-blue-600 text-white font-semibold h-10 px-6 shadow-sm hover:shadow-md transition-all disabled:opacity-50 flex items-center gap-2"
     >
