...
function renderMarkdown(text: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('## ')) {
      elements.push(
        <h2 key={key++} className="text-base font-semibold text-white mt-3 mb-1">
    ...
    if (trimmed.startsWith('# ')) {
      elements.push(
        <h1 key={key++} className="text-lg font-bold text-white mt-3 mb-1.5">
  ...
  return parts.map((part, i) => {
    ...
    return codeParts.map((cp, j) => {
      if (cp.startsWith('`') && cp.endsWith('`')) {
        return (
          <code key={`${i}-${j}`} className="px-1.5 py-0.5 rounded bg-slate-800 text-emerald-300 text-xs font-mono">
            {cp.slice(1, -1)}
          </code>
        )
      }
...
function TypingIndicator() {
  return (
    <div className="flex items-start gap-3 px-4 py-2">
      <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center shrink-0 shadow-lg shadow-violet-500/20">
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
      <div className="relative mb-6">
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-600 via-fuchsia-600 to-pink-600 flex items-center justify-center shadow-2xl shadow-violet-500/30">
      ...
      <h2 className="text-xl font-bold text-white mb-2">TITAN AI Consultant</h2>
      <p className="text-slate-400 text-sm text-center max-w-md mb-8 leading-relaxed">
...
export function AIAssistantView() {
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="flex h-[calc(100vh-5rem)] rounded-xl border border-slate-800 bg-slate-950 overflow-hidden">
        {/* Sidebar — hidden on mobile, visible on md+ */}
        <div
          className={`hidden md:flex flex-col w-64 border-r border-slate-800 bg-slate-900/50 transition-all duration-300 ${
        ...
        >
          <div className="p-4 flex items-center justify-between border-b border-slate-800">
            ...
            <Tooltip>
              ...
              <TooltipContent side="right" className="bg-slate-800 text-white border-slate-700">Collapse sidebar</TooltipContent>
            </Tooltip>
          </div>
          ...
          <div className="p-3 border-t border-slate-800">
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-slate-800 transition-colors disabled:opacity-40 disabled:pointer-events-none"
            >
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {/* Top bar */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-900/30 shrink-0">
            <div className="flex items-center gap-2">
              {!sidebarOpen && (
                <Tooltip>
                  <TooltipTrigger asChild>
                  ...
                  <TooltipContent side="right" className="bg-slate-800 text-white border-slate-700">Open sidebar</TooltipContent>
                </Tooltip>
              )}
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                <h1 className="text-sm font-semibold text-white">AI Strategy Consultant</h1>
              </div>
            </div>
          ...
          <div className="md:hidden flex gap-2 px-4 py-2 border-b border-slate-800 overflow-x-auto shrink-0">
            {QUICK_ACTIONS.map((action) => (
              <button
                key={action.id}
                onClick={() => handleQuickAction(action.prompt)}
                disabled={isTyping}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border ${action.borderColor} bg-gradient-to-r ${action.gradient} shrink-0 text-xs text-slate-300 hover:text-white transition-colors disabled:opacity-50`}
              >
              ...
              <>
                      ...
                      >
                        ...
                        <div
                          className={`max-w-[75%] sm:max-w-[70%] rounded-2xl px-4 py-3 ${
                            msg.role === 'user'
                              ? 'bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white rounded-tr-md shadow-lg shadow-violet-500/20'
                ...
                {showScrollBtn && (
                  <button
                    ...
                    className="absolute bottom-4 right-4 w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-700 transition-all shadow-lg"
                  >
            ...
            <form onSubmit={handleSubmit} className="flex items-end gap-3 max-w-4xl mx-auto">
              <div className="flex-1 relative">
                <Textarea
                  ...
                  disabled={isTyping}
                  rows={1}
                  className="min-h-[44px] max-h-[120px] resize-none bg-slate-900 border-slate-700 text-white placeholder:text-slate-500 rounded-xl px-4 py-3 pr-12 text-sm focus-visible:ring-violet-500/50 focus-visible:border-violet-500/50 transition-all"
              ...
              <Button
                ...
                className="h-11 w-11 p-0 rounded-xl bg-gradient-to-br from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 shadow-lg shadow-violet-500/20 shrink-0 disabled:opacity-40 disabled:shadow-none"
              >
