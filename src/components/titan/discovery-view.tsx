...
function renderStars(rating: number) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <span className="text-xs text-gray-400 ml-1">{rating > 0 ? rating.toFixed(1) : '—'}</span>
    </div>
  )
}
  ...
  return (
    <motion.div className="mx-auto max-w-5xl space-y-6" variants={container} initial="hidden" animate="show">
      ...
      <motion.div variants={item}>
        <h1 className="text-2xl font-bold tracking-tight text-gray-900">Lead Discovery</h1>
      ...
      >
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9 h-10 rounded-lg bg-gray-50 border-gray-200/60 text-gray-900 placeholder:text-gray-400 focus-visible:ring-1 focus-visible:ring-blue-600/20 focus-visible:border-blue-300"
                disabled={searching}
              />
            </div>
          </div>
          ...
          <Button
            ...
            className="h-10 px-5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg shadow-sm disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
          >
            {searching ? (
              <>
        ...
        {searchError && (
          <div className="mt-3 p-3 rounded-lg bg-red-50 border border-red-200/60 text-sm text-red-600 flex items-start gap-2">
      ...
      {searching && (
        <motion.div
          variants={item}
          className="bg-white rounded-xl border border-gray-200/60 p-8"
        >
          <div className="flex flex-col items-center justify-center py-6 space-y-4">
            ...
            <div className="text-center">
              <p className="text-gray-900 font-medium text-sm">Searching for businesses...</p>
            ...
            <div className="flex gap-3 mt-2">
              {Array.from({ length: 3 }).map((_, i) => (
                <Skeleton key={i} className="h-10 w-44 rounded-lg bg-gray-100" />
              ))}
            </div>
          </div>
        </motion.div>
      ...
      {leads.length > 0 && !searching && (
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-4">
          {/* Summary bar */}
          <motion.div variants={item} className="flex flex-wrap items-center gap-3 px-1">
            <span className="text-sm font-medium text-gray-900">{totalResults} businesses found</span>
            <Badge variant="outline" className="bg-emerald-50 border-emerald-200/60 text-emerald-700 text-xs font-medium rounded-md">
              {newCount} new
            </Badge>
            <Badge variant="outline" className="bg-gray-50 border-gray-200/60 text-gray-500 text-xs font-medium rounded-md">
              {existingCount} already in CRM
            </Badge>
          </motion.div>

          {/* Desktop Table */}
          <motion.div variants={item} className="bg-white rounded-xl border border-gray-200/60 hidden md:block overflow-hidden">
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50/80 border-b border-gray-200/60 hover:bg-gray-50/80">
                    <TableHead className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 py-3">Business</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-3 py-3">Phone</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-3 py-3">Website</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-3 py-3">Rating</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-3 py-3">Status</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-3 py-3 text-right">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {leads.map((lead, idx) => (
                    ...
                    >
                      ...
                      <TableCell className="px-3 py-3">
                        {isAdded(lead) ? (
                          <Badge variant="outline" className="bg-gray-50 border-gray-200/60 text-gray-500 text-[10px] font-medium rounded-md">
                            <Check className="w-2.5 h-2.5 mr-1" />
                            In CRM
                          </Badge>
                        ) : (
                          <Badge variant="outline" className="bg-emerald-50 border-emerald-200/60 text-emerald-600 text-[10px] font-medium rounded-md">
                            New
                          </Badge>
                        )}
                      </TableCell>
                      <TableCell className="px-3 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            ...
                            className="h-8 px-3 text-xs border-gray-200/60 text-gray-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200/60 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg"
                          ...
                          <Button
                            ...
                            className={cn(
                              'h-8 px-3 text-xs transition-colors disabled:opacity-60 disabled:cursor-not-allowed rounded-lg',
                              isAdded(lead)
                                ? 'border-emerald-200/60 bg-emerald-50 text-emerald-600 cursor-default'
                                : 'border-gray-200/60 text-gray-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200/60',
                            )}
                          >
                            {isAdded(lead) ? (
                              <>
          ...
          <div className="md:hidden space-y-3">
            {leads.map((lead, idx) => (
              <motion.div
                key={lead.placeId || idx}
                variants={item}
                className="bg-white rounded-xl border border-gray-200/60 p-4 space-y-3"
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  {isAdded(lead) ? (
                    <Badge variant="outline" className="bg-gray-50 border-gray-200/60 text-gray-500 text-[10px] font-medium shrink-0 rounded-md">
                      <Check className="w-2.5 h-2.5 mr-1" />
                      In CRM
                    </Badge>
                  ) : (
                    <Badge variant="outline" className="bg-emerald-50 border-emerald-200/60 text-emerald-600 text-[10px] font-medium shrink-0 rounded-md">
                      New
                    </Badge>
                  )}
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  <Button
                    ...
                    onClick={() => handleAudit(lead)}
                    disabled={!lead.website}
                    className="flex-1 h-8 text-xs border-gray-200/60 text-gray-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200/60 disabled:opacity-40 rounded-lg"
                  >
                    <Sparkles className="w-3 h-3 mr-1" />
                    Audit
                  </Button>
                  <Button
                    ...
                    disabled={isAdded(lead)}
                    className={cn(
                      'flex-1 h-8 text-xs transition-colors disabled:opacity-60 rounded-lg',
                      isAdded(lead)
      ...
      {!searching && leads.length === 0 && searchError === '' && totalResults === 0 && (
        <motion.div
          variants={item}
          className="bg-white rounded-xl border border-dashed border-gray-200/60 p-12 text-center"
        >
          <div className="w-12 h-12 rounded-xl bg-gray-50 border border-gray-200/60 flex items-center justify-center mx-auto mb-4">
            <Radar className="w-5 h-5 text-gray-300" />
          </div>
          <p className="text-gray-900 font-medium text-sm">Search for businesses on Google Maps</p>
      ...
      {!searching && searchError === '' && leads.length === 0 && totalResults > 0 && (
        <motion.div
          variants={item}
          className="bg-white rounded-xl border border-gray-200/60 p-12 text-center"
        >
          <div className="w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center mx-auto mb-4">
            <Search className="w-5 h-5 text-gray-300" />
          </div>
          <p className="text-gray-900 font-medium text-sm">No businesses found</p>
          <p className="text-xs text-gray-400 mt-1">
      ...
      <Dialog open={auditModalOpen} onOpenChange={(open) => { if (!open) { setAuditModalOpen(false); setAuditResult(null); setAuditError('') } else { setAuditModalOpen(true) } }}>
        <DialogContent className="bg-white border-gray-200/60 shadow-xl sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col p-0 rounded-xl">
          {auditLead && (
            <>
              ...
              <div className="p-5 pb-0">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-xs text-gray-400 mt-1">
                    {auditLead.name}
                    {auditLead.website && (
                      <a href={auditLead.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline ml-1.5">
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                {auditLoading && (
                  <div className="p-6 space-y-4">
                    <div className="flex flex-col items-center justify-center py-8 space-y-4">
                      ...
                      <div className="text-center">
                        <p className="text-gray-900 font-medium text-sm">Analyzing website...</p>
                        <p className="text-xs text-gray-400 mt-1">AI is auditing design, technical, business & automation</p>
                      </div>
                    </div>
                ...
                {!auditLoading && auditError && (
                  <div className="p-5">
                    <div className="p-4 rounded-lg bg-red-50 border border-red-200/60 flex items-start gap-3">
                      <AlertTriangle className="w-4 h-4 text-red-500 mt-0.5 shrink-0" />
                      <div>
                        <p className="text-sm font-medium text-red-700">Audit Failed</p>
                        <p className="text-xs text-red-500 mt-1">{auditError}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {!auditLoading && auditResult && (
                  <div className="p-5 pt-2 space-y-5">
                    ...
                    <div className="flex items-center gap-4">
                      <div className={cn('w-16 h-16 rounded-xl border-2 flex flex-col items-center justify-center shrink-0', scoreBg(auditResult.overallScore))}>
                        <span className={cn('text-xl font-bold leading-none', scoreColor(auditResult.overallScore))}>
                    ...
                    {auditResult.scores && Object.keys(auditResult.scores).length > 0 && (
                      <div className="grid grid-cols-2 gap-2.5">
                        {Object.entries(auditResult.scores).map(([key, score]) => {
                          ...
                          return (
                            <div key={key} className="p-3 rounded-lg bg-gray-50 border border-gray-200/40 space-y-1.5">
                    ...
                    <div className="pt-2 pb-1">
                      <Button
                        ...
                        className="w-full h-9 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg"
                      >
